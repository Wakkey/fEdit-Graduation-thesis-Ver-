...
uses interfaceunit, functionunit, mainform;
...
begin
  ...
  function_unit.editlist.Items[function_unit.TabpageNo].SaveDialog1.FileName:= 'sanksform.php';
  function_unit.editlist.Items[function_unit.TabpageNo].SaveDialog1.DefaultExt:= '.php';
    try
      interface_unit.saveas(main.PageControl1.ActivePageIndex);
      save_filePHPname := extractfilename(function_unit.editlist.Items[function_unit.TabpageNo].filename_path);
    except
...
begin
  ...
  st.Clear;
  With function_unit.editlist.Items[function_unit.TabpageNo] do //.Items do
    begin
    ...
    with Json_write do begin
      try
        ...
        s1 := extractfilepath( function_unit.editlist.Items[main.PageControl1.ActivePageIndex].filename_path );
        s2 := ( function_unit.editlist.Items[main.PageControl1.ActivePageIndex].filename_path );
        ...
        s2 := s2 + 'form.php';
        SaveToFile(s2);
        interface_unit.macro_run(s2);

        With function_unit.editlist.Items[function_unit.TabpageNo].TreeView1 do begin
  ...
  With function_unit.editlist.Items[function_unit.TabpageNo] do //.Items do
    ...
    with Json_write do begin
      try
        ...
        val := inttostr(function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Count);
    ...
    Case Data.JSONType of
      ...
      jtObject:begin
        If (Data.JSONType=jtArray) then begin
         ...
         //ツリー構造の記述
        //function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Add(
        //'<fieldset  id="btn">'+
        //'<dl id="tgl_menu">'
       // );
        //function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Add('<dt id='+AParent.GetTextPath+' for="'+AParent.GetTextPath+'"><button type="button">'+AParent.GetTextPath+'</button></dt>');
        //function_unit.editlist.Items[function_unit.TabpageNo].SynEdit1.Lines.Add('<legend>'+ Data.AsString +'</legend>');
        N:=function_unit.editlist.Items[function_unit.TabpageNo].TreeView1.Items.AddChild(AParent,Format(C,[Data.Count]));
        ...
        try
          ...
          For I:=0 to S.Count-1 do begin
            //ルート部分枝表示
            N2:=function_unit.editlist.Items[function_unit.TabpageNo].TreeView1.Items.AddChild(N,S[i]);
            ...
            ShowJSONData(N2,D);
            try
              function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Add(Jsondate_to_htmlobj(ImageTypeMap[D.JSONType],d.AsString,ExtractFileName(D.AsString),AParent.GetTextPath));
            except
       ...
       // function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Add('</fieldset>');
        //showmessage(AParent.GetTextPath);
        if -1 < set_JsonTitle.IndexOf(AParent.GetTextPath) then begin
          //showmessage(set_JsonData.Text);
        end;
        function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Add(Jsondate_to_htmlobj(ImageTypeMap[Data.JSONType],'','',''));

      end;
      jtNull:begin
        N:=function_unit.editlist.Items[function_unit.TabpageNo].TreeView1.Items.AddChild(AParent,SNull);
        //showmessage(AParent.ToString);
        function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Add(AParent.ToString);
      end
      else begin
        //値表示
        ...
        function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Add(Jsondate_to_htmlobj(ImageTypeMap[Data.JSONType],data.AsString,ExtractFileName(Data.AsString),AParent.GetTextPath));
        N:=function_unit.editlist.Items[function_unit.TabpageNo].TreeView1.Items.AddChild(AParent,Data.AsString);
      end;
    end;
    If Assigned(N) then begin
      //function_unit.editlist.Items[function_unit.TabpageNo].SynEdit1.Lines.Add(Jsondate_to_htmlobj(ImageTypeMap[Data.JSONType],data.AsString));
                        ...
                        //Jsondate_to_htmlobj := '<dd>';
                             if set_obj_type = 'string' then begin
                              ...
                              a1 := set_obj_type + inttostr(htmobj_count);
                                ...
                                if ( example_st.Count > 0 ) and ( 0 = ansipos(S_title,function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Text)) then begin
                              ...
                              if ( example_st.Count > 0 ) and (set_obj_type = 'integer') or (set_obj_type = 'number') then begin
                                ...
                                s2 := '<dd><div ng-init="'+JSON_output[0]+'.'+a1+' = '''+example+'''"><label>'+S_title+'<br>'+S_description+'</label><p><input type="number" name="'+a1+'" ng-init="'+example+'" ng-model="'+JSON_output[0]+'.'+a1+'" required '+i_short+' '+i_long+'><br><span ng-show="JSONForm.'+a1+'.$error.required">required</span><br><span ng-show="JSONForm.'+a1+'.$error.min">too short</span><span ng-show="JSONForm.'+a1+'.$error.max">too long</span></p></dd>';
                                 if 0 = ansipos(S_title,function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Text) then begin
                                   set_comp_name.Add(a1);
                                   set_caption.Add(S_title);
                                   Jsondate_to_htmlobj := Jsondate_to_htmlobj + s2;
                              end;
                      ...
                      'チェックボックス':begin
                        ...
                        if ( example_st.Count > 0 ) and ( 0 = ansipos(S_title,function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Text) ) then begin
                          ...
                          for i := 0 to example_st.Count -1 do begin
                            //if 0 < ansipos(example_st[i],function_unit.editlist.Items[function_unit.TabpageNo].Json_write.Text) then begin
                            set_comp_name.Add(a1);
                            set_caption.Add(S_title);
...
begin
  st := TStringlist.Create;
  st.Text:= function_unit.editlist.Items[function_unit.TabpageNo].SynEdit1.Text;

  st2 := TStringlist.Create;
  st2.Clear;
  interface_unit.JsonParse(st,st2,edit1.Text);
  memo1.Text:= st2.Text;
...
begin
  ...
  st3.Clear;
  Json_pars.J:=GetJSON(function_unit.editlist.Items[function_unit.TabpageNo].SynEdit1.Text);
   function_unit.json_Keword_parse(st,st2);
